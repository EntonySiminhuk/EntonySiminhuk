...
int conta_vermelhos(NodeVP *tree){
    if (tree != NULL){
        return (tree->color == 0) + conta_vermelhos(tree->left) + 
                conta_vermelhos(tree->right);
    }
    
    return 0;
}

int conta_Pretos(NodeVP *tree)
{
    if (tree != NULL)
    {
        return (tree->color == 1 && tree->key % 2 != 0) + conta_Pretos(tree->left) + conta_Pretos(tree->right);
    }
