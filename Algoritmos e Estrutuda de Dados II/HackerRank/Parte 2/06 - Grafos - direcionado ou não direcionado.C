...
/***************************************************************/
GrafoMA *converter_LA_MA(GrafoLA *gLA)
{
    int i;
    Cell *aux;
    ...
    else if (gLA != NULL)
    {
        for (i = 0; i < gLA->V; i++)
        {
            ...
            while (aux != NULL)
            {
                inserir_arestaMA(gMA, i, aux->key);
                aux = aux->next;
            }
        }
    }
...
int eh_direcionado(GrafoLA *gLA)
{
    int j, i;
    if (gLA != NULL)
    {
        GrafoMA *gMA = converter_LA_MA(gLA);
...
int main()
{
    int vert_s, vert;
    scanf("%d", &vert_s);
    GrafoLA *gLA = iniciar_grafoLA(vert_s);
    ...
    for (int i = 0; i < vert_s; i++)
    {
        do
        {
            scanf("%d", &vert);
            if (vert != -1)
                inserir_arestaLA(gLA, i, vert);

        } while (vert != -1);
    }

    if (eh_direcionado(gLA) == 1)
        printf("direcionado");
    else
